...
Ist „weil ich es kann“ eine ausreichende Antwort? Nein, wahrscheinlich nicht. „weil ich es wollte“? Immer noch nicht? Nun gut, das muss wohl reichen. Der eigentliche Grund ist, dass ich [docling](https://docling-project.github.io/docling/) verwenden musste, um einige Dokumente von einem Format in ein anderes zu konvertieren. Ich hatte immer [pandoc](https://pandoc.org) verwendet, aber da ich nicht mehr bei [Red Hat](https://redhat.com) arbeite und wir uns intensiv mit Docling beschäftigen, dachte ich, ich probiere es einfach mal aus.

Ähnlich wie Pandoc ist Docling größtenteils (eigentlich ausschließlich) über die Kommandozeile bedienbar. Das ist zwar für viele Zwecke gut geeignet, insbesondere bei der Automatisierung von Prozessen, aber es ist schwierig, sich alle Kommandozeilenoptionen zu merken, wenn man sie nicht häufig nutzt.

Das wird ein längerer Text, aber wenn Sie sich nicht für die Details interessieren, können Sie vieles überspringen. Vor allem, wenn Sie nur sehen möchten, wie die neue, schicke Benutzeroberfläche aussieht und wie sie funktioniert.

Da ich für die Entwicklung dieses Projekts einen Coding-Assistenten verwendet habe, werde ich auch den Prozess erläutern, den ich für Coding-Assistenten anwende.

## Schauen wir uns Pandoc an

Mein bewährtes Programm zum Konvertieren von Markdown-Dateien in `.docx`-Dateien war schon immer pandoc. Aber jedes Mal muss ich mich erst wieder daran erinnern, wie es funktioniert. Hier sind die Befehlszeilenparameter für pandoc:
...
Viel Glück beim Erinnern all (oder auch nur eines davon). Autsch!

Es funktioniert super, und ich denke, wenn ich es jeden Tag benutzen würde, könnte ich mir zumindest genug merken, um Standardumrechnungen ziemlich schnell durchzuführen. Aber das tue ich nicht, und das kann ich auch nicht.
