...
J'ai créé **Duckling**, une interface graphique pour la documentation qui vous évite d'avoir à mémoriser plus de 40 options en ligne de commande. Si vous voulez juste voir la [nouvelle interface utilisateur](#the-front-end), vous pouvez passer directement à la suite. Sinon, restez pour découvrir comment j'ai utilisé des assistants de programmation IA pour développer cet outil, et ce que j'ai appris en cours de route.

Liens rapides pour en savoir plus : [Dépôt GitHub](https://github.com/davidgs/duckling) | [Documentation en ligne](https://duckling-ui.org) | [Avertissement de sécurité](#limitations)
